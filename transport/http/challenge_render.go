...
// Render streams a challenge page without reflection or an intermediate page
// buffer. payload is encoding/json output and therefore already safe for the
// application/json script element. id is generated by Guardian; EscapeString
// is a zero-allocation no-op for both supported alphabets and remains safe if a
// future ID format introduces an HTML-special character.
func (r *challengeRenderer) Render(w io.Writer, payload []byte, noScript bool, refreshSeconds, id string) error {
	if !noScript {
		if err := writeChallengeBytes(w, r.js[0]); err != nil {
			return err
		}

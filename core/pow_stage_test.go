...
func TestPoWStages(t *testing.T) {
	...
	cases := []struct {
		...
		// The next three are all re-challenged, but each names its own cause:
		// without that split an operator staring at a re-challenge loop cannot
		// tell a cookie that never arrived from one that arrived and was
		// rejected, which is the only question worth asking (see #43/#47).
		{"garbage token is re-challenged as invalid",
			&RequestContext{Host: "html.test", Method: "GET", URI: "/", RemoteAddr: ip, UserAgent: ua,
				Cookie: pow.CookieName + "=garbage"},
			ActionChallenge, "pow:token_invalid"},
		{"empty token cookie is present but invalid",
			&RequestContext{Host: "html.test", Method: "GET", URI: "/", RemoteAddr: ip, UserAgent: ua,
				Cookie: pow.CookieName + "="},
			ActionChallenge, "pow:token_invalid"},
		{"token from another IP is re-challenged as a binding mismatch",
			&RequestContext{Host: "html.test", Method: "GET", URI: "/", RemoteAddr: "203.0.113.1", UserAgent: ua,
				Cookie: pow.CookieName + "=" + token},
			ActionChallenge, "pow:token_binding"},
		{"token from another User-Agent is re-challenged as a binding mismatch",
			&RequestContext{Host: "html.test", Method: "GET", URI: "/", RemoteAddr: ip, UserAgent: "curl/8.0",
				Cookie: pow.CookieName + "=" + token},
			ActionChallenge, "pow:token_binding"},

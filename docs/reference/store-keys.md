...
| `ev:` | Per-IP behaviour counters, one key per event type, unblock generation and window bucket | Progress toward an `ip_behaviour` threshold resets | Not surfaced; the current generation is rotated and its old keys are cleared by an unblock |

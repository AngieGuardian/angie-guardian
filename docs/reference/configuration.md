...
| `admin.recent_size` | int | `4096` | Capacity of the per-instance, in-memory ring of non-allow decisions and solved challenges, used by recent stats, offenders and dashboard charts. Solves share the ring, so on a site where most challenges are solved a given size reaches back over less decision history; under attack they are a negligible share, since bots do not solve. Range `1..16384`. It is scanned by live admin reports, cleared on restart, and start-time-only (a reload that changes it is rejected). Use `/metrics` with Prometheus/Grafana for historical analysis. |

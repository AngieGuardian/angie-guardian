...
| `admin.recent_size` | int | `4096` | Capacity of the per-instance, in-memory non-allow decision ring used by recent stats, offenders and dashboard charts. Range `1..16384`. It is scanned by live admin reports, cleared on restart, and start-time-only (a reload that changes it is rejected). Use `/metrics` with Prometheus/Grafana for historical analysis. |

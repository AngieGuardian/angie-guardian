...
|---|---|---|---|
| `listen` | string | `127.0.0.1:8071` | Numeric `host:port` for the auth hot path (Angie's `auth_request` target). Must be loopback unless `trusted_proxy` is set. Listener syntax is checked by `guardiand -t`. When neither endpoint field is present, both default endpoints are enabled. Explicitly configuring only `socket` disables TCP. |
| `socket` | string | `/run/guardian/guardian.sock` | Absolute Unix-domain socket path for the auth hot path. When neither endpoint field is present, both default endpoints are enabled. Explicitly configuring only `listen` disables the socket; configure both fields to serve both. Guardian refuses to replace a regular file or active socket, removes an orphaned socket after an unclean exit, applies `socket_mode`, and removes it on clean shutdown. The immediate parent, when it exists at preflight, must be a directory that is not group- or world-writable. |
| `socket_mode` | string | `"0660"` | Quoted four-digit octal permission applied to the Unix socket at startup. The shipped socket is `guardian:guardian`; default `0660` restricts access to that user and group. Setting `"0666"` avoids changing Angie's worker groups, but allows every local process to connect and forge the `X-Guardian-*` identity headers Guardian trusts from Angie. Start-time only. |
...
| `previous_key_dir` | string | | Where retired signing keys (from `POST /admin/rotate-key`) are archived; required for rotation. Replicas must share it with `signing_key_file`. Retired keys accept only pre-rotation tokens with lifetimes up to thirty days; older archives may remain on disk but are omitted from the active verification set. |
| `admin` | object | | See [admin](#admin). |
| `store` | object | | See [store](#store). |
| `argon2_verifier.max_concurrent` | int | `1` | Global maximum simultaneous Argon2id verifications. Range 1..16. This is global because domains share CPU, heap, and memory bandwidth. SHA-256 does not enter this pool. |
| `argon2_verifier.verification_rate_limit` | rate | `10/min` | Pre-verification cap per host+IP and, when `geoip.asn_db` is configured, per host+ASN. Applied after cheap challenge authentication and before Argon2id work. |
...
| `admin.metrics_auth` | bool | `false` | Require the admin bearer token on `GET /metrics`. Worth enabling on a routable `admin.listen`: metrics expose every protected vhost name plus per-domain traffic and attack posture. `/healthz` and `/readyz` always stay open for orchestrators. Pair with a stable `token` or `token_file`; Prometheus scrapes with `authorization.credentials_file`. |
| `admin.dashboard` | bool | `false` | Serve the built-in reporting page at `GET /admin/dashboard`. On startup guardiand logs the bare URL; enter the token into the login gate. Configured and persistent bearer tokens are never embedded in logs. |
| `admin.recent_decisions_capacity` | int | `4096` | Capacity of the per-instance, in-memory ring of non-allow decisions and proof-of-work outcomes (solves, failed redemptions and recovered network handovers), used by recent stats, offenders and dashboard charts. Outcomes share the ring, so on a site where most challenges are solved a given size reaches back over less decision history; under attack solves are a negligible share, since bots do not solve. Range `1..16384`. It is scanned by live admin reports, cleared on restart, and start-time-only (a reload that changes it is rejected). Use `/metrics` with Prometheus/Grafana for historical analysis. |
| `admin.angie_api.url` | string | (empty = disabled) | Base URL of Angie's [HTTP API][angie-api] location (e.g. `http://127.0.0.1:81/status/`). Enables the dashboard's **Server traffic** panels: per-domain requests, connections, response codes and bandwidth Guardian never sees on its allow path, plus upstream peer health, proxy-cache hit rates, Angie's own rate-limit zones and shared-memory pressure. guardiand reads it server-side and relays only a fixed set of status endpoints behind the admin token, so keep the API on loopback. See [Enabling the Angie API](/guide/admin#enabling-the-angie-api). |
...
| `action` | `deny` \| `challenge` | `deny` | `deny` rejects matching IPs outright; `challenge` makes them solve PoW first, one full step (+4 bits = 16x) above base, like a WAF rule hit. Challenge feeds are inert on a PoW-disabled domain. |
...
- Matching is byte-exact and case-sensitive. The request path is normalized
  first (dot-segments and duplicate slashes resolved, the same form Angie
  serves), so `/static/../admin` cannot adopt or escape an override. Keys must
  therefore be written in that normalized form as well: `/api//v1/` and
  `/a/./b` could never match anything and are rejected at load rather than
  becoming silently dead overrides. Keys must start with `/` and cannot contain
  `?` or `#`.
...
- A WAF rule with `action: challenge` degrades to a deny on a path whose
...
`algorithm` and `argon2id` are defaults/domain scoped and cannot be changed by
a `paths:` overlay. See [Proof-of-work algorithms](/guide/pow-algorithms) for
the security and capacity trade-offs.

| Option | Type | Default | Description |
|---|---|---|---|
| `pow.enabled` | bool | `false` | Enable the proof-of-work challenge layer for this domain. Requires top-level `signing_key_file`. |
| `pow.algorithm` | `sha256` \| `argon2id` | `sha256` | Challenge algorithm. `sha256` is the recommended default and is cheap for Angie Guardian to verify. With `argon2id`, Angie Guardian recomputes the same memory-hard proof as the browser. Defaults/domain scope only. |
...
| `pow.base_difficulty` | float | `5` | SHA-256 baseline. Must be finite and in range 1..8, in quarter steps. A difficulty of `N` requires `4 * N` leading zero bits: +1 is 16x the work, +0.25 is one bit (2x). For Argon2id, escalation selects the bounded iteration ladder instead. |
| `pow.max_difficulty` | float | `6` | SHA-256 normal-operation escalation ceiling. For Argon2id, the same suspicion/escalation signal selects `argon2id.max_iterations`; attack posture selects its hard cap. Must remain valid even when Argon2id is selected. |
| `pow.argon2id.memory_kib` | int | `32768` | Memory for one fixed-result Argon2id computation. Range 8192..32768 KiB. Defaults/domain scope only. |
| `pow.argon2id.base_iterations` | int | `1` | Ordinary-client iterations. Range 1..3. |
| `pow.argon2id.max_iterations` | int | `2` | Suspicious/escalated-client iterations. Range `base_iterations`..3. |
| `pow.argon2id.attack_iterations_cap` | int | `3` | Hard iteration cap selected in attack posture. Range `max_iterations`..3. |
| `pow.token_ttl` | Duration | `7d` | Lifetime of the signed JWT cookie a solved challenge earns: how long a clean visitor rides one solve before being challenged again. Two levers shape the work economy: `base_difficulty` sets the cost of one solve, `token_ttl` sets how often anyone still visiting, a legitimate client or a persistent scraper alike, pays that cost again, and it also bounds how long a captured cookie stays replayable. The token is fingerprint-bound to IP + User-Agent, so replay needs the same source IP and UA; that binding is weakest behind shared IPs (CGNAT, mobile carriers, corporate proxies), where any client on the same IP that copies the cookie and UA can ride it. A shorter TTL re-challenges legitimate visitors more often, but raises a continuing scraper's amortized cost and shrinks that replay window: the classic UX-versus-abuse-resistance trade-off. `7d` is the default middle ground for most sites; consider shorter values when shared-IP replay worries you. Must be between `1s` and thirty days when PoW is enabled; if you rotate signing keys, keep it well under thirty days so a rotation does not invalidate live tokens. |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `enabled` | bool | `false` | Enable event counting and automatic/persistent blocks from WAF rules, honeypots, failed PoW, tamper, and bot spoofing. Existing and manually placed blocks are still enforced when disabled. |
| `block_ttl` | Duration | `30m` | First-offense block duration; doubles per repeat offense, so the default pair walks `30m` -> `1h` -> `2h` -> `4h` -> `8h` -> `12h`. The repeat counter behind the doubling ([`blkct:`](/reference/store-keys)) is kept for 24h, so an IP that stays quiet for a day starts again at `30m` rather than where it left off. Maximum one year (`8760h`). |
| `max_block_ttl` | Duration | `12h` | Backoff cap, reached on the sixth block within 24h under the default `block_ttl`. Raise both together: lifting this alone only adds rungs high on the ladder, which an IP can only reach by being blocked six or seven times inside the 24h counter window. Must be >= `block_ttl`; maximum one year (`8760h`). |
| `thresholds` | map of Rate | `rule_match: 10/min`, `pow_fail: 10/min`, `tamper: 10/min`, `bot_spoof: 5/min`, `challenge_farm: 80/h` | Bad events per window before the IP is blocked, keyed by event type: `rule_match` counts WAF rule hits, `pow_fail` wrong submitted PoW solutions, `tamper` forged or replayed challenge/signed IDs, `bot_spoof` clients claiming a verified-bot User-Agent whose rDNS identity failed verification, and `challenge_farm` challenges fetched but never solved, counted only once the client's [unsolved-challenge escalation](/guide/configuration#base-difficulty-and-max-difficulty) is pinned at `max_difficulty` (any successful solve resets the counter, so real visitors never accumulate events; tighten to e.g. `30/h` when farmers are a problem). Built-in defaults merge per key (your value wins); the literal `off` disables one event type individually. Configurable in `defaults` and per domain; events are host-scoped, so a `paths:` overlay cannot change them. |

Failed no-JS GET redemptions are rejected and recorded, but do not add
`pow_fail` or `tamper` events because a third-party site can induce those
requests from a visitor's browser. Failed POSTed solutions still score.
The POST endpoint requires `Content-Type: application/json`; cross-site form
content types are rejected before redemption or scoring.

### waf.rules

| Option | Type | Default | Description |
|---|---|---|---|
| `enabled` | bool | `false` | Enable WAF rules with literal/regex matchers. Requires at least one effective entry in `files`. Rules match the targets they name: `path`, `query` (the default pair), `ua`, or `header:<name>` (e.g. `header:accept`, `header:referer` or `header:host`; Host uses Angie's effective `X-Guardian-Host`). All inputs are lowercased; paths, queries, and targeted header values are also percent-decoded, while User-Agent values are not decoded. Every physical value of a duplicate header is inspected, and the first match in effective file order then document order wins. Actions are `allow`, `deny` (the default), `challenge` and `block`. `allow` is terminal at the WAF stage and emits no bad-event score: earlier denylist, deny-intel, active-block and honeypot stages still win, while later PoW, challenge-intel and anomaly stages are skipped. `methods: [ TRACE, TRACK ]` restricts a rule to those HTTP methods. In the sidecar, a valid bound PoW token satisfies an `action: challenge` match; without PoW that action denies. `deny` always terminates, while `block` persists only when `waf.ip_behaviour.enabled`. The stateless WASM guest returns allow for `allow`; without challenge or block state, the other three matching actions return deny. Empty or whitespace-only keywords and regexes are rejected. |
| `files` | list of string | `[]` | Ordered rules files (start with [`deploy/rules-common.yaml`](https://github.com/AngieGuardian/angie-guardian/blob/main/deploy/rules-common.yaml), which documents every field). Unlike ordinary list overlays, this list is cumulative: defaults run first, domain files append, and matching-path files append last. Each file must contain exactly one YAML document, be no larger than 8 MiB, and exist when enabled (fail-fast); files are hot-reloaded independently while each effective combined set is published precompiled. Empty or duplicate paths and duplicate rule IDs across an effective set are rejected. An oversized or invalid update keeps the last-good combined policy active, as does an update that removes or renames a rule ID still listed in some scope's `disabled_ids`. |
| `disabled_ids` | list of string | `[]` | Exact, case-sensitive rule `id`s to remove from evaluation against this scope's combined effective `files`, without copying them. The remaining rules keep effective file and document order, so the next matching rule still decides; a disabled rule produces no decision, log line or metric. Overlays replace the list wholesale: omitted inherits the parent's resolved list, an explicit `[]` clears it, a non-empty list replaces it (re-list inherited IDs to keep them). Effective filtered rule sets are precompiled per scope at startup/reload; there is no per-request cost. Validation is strict everywhere configuration enters the runtime (`-t`, startup, config reload, watched-file reload): empty/whitespace/duplicate entries are rejected, a non-empty list requires effective `files` even while `enabled: false`, and every ID must exist in that scope's combined set, with errors naming the scope, files and unknown ID. To delete a disabled rule from a file, first drop its ID here and reload, then edit the file. |
...
| `model` | string | | Path to the model artifact from `guardian-train`. Required when enabled, limited to 64 MiB, and hot-swapped when the file changes. Every host listed under `domains` whose resolved policy enables anomaly scoring must have a baseline in the artifact, including when it inherits that enablement from `defaults`. An oversized, invalid, or incomplete update keeps the last-good model active. |
